...
sample_every = 250  # every how many steps to sample from the model
save_every = (
    -1
...
while True:
    ...
    for running_idx in range(supervision_steps):
        for ind_idx, (x_ind, y_ind, z_ind, target) in enumerate(
            zip(accum_x, accum_y_inner, accum_z_inner, accum_target)
        ):
            input = x_ind.clone()
            y_ind_new, z_ind_new, output, q_stop = model.deep_recursion(
                input, y_ind, z_ind
            )
            accum_y_inner[ind_idx] = y_ind_new
            accum_z_inner[ind_idx] = z_ind_new
            loss = model.get_loss(output, q_stop, target)
